...
class BoardEncoding(gym.ObservationWrapper):
    ...
    def __init__(self, env, history_length) -> None:
    ...
    def reset(self) -> np.array:
        ...
        self._history.reset()
        return super(BoardEncoding, self).reset()

##############################################################################
    def new_state(self, new_state, board):
        self._history.new_state(new_state)
        super(BoardEncoding, self).new_state(board)        
        
    
    def buffer_ob(self):
        return self._history._buffer        
    
    
    def board_ob(self):
        return super(BoardEncoding, self).board_ob()   
...
#    def clean(self, new_state: np.array([8, 8, 112], dtype=np.int)):
#        self._buffer = new_state
